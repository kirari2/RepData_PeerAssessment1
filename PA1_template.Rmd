...
df1 <- df %>% 
        group_by(date) %>% 
        summarise(total_steps = sum(steps, na.rm = TRUE))
print(paste("The mean and median number of steps taken each day are", round(mean(df1$total_steps)), "and", median(df1$total_steps), "respectively."))
```
...
```{r}
print(paste("The 5-Minute interval that contained the most steps on average is", df2[which.max(df2$avg_step), ]$interval, "with",  round(df2[which.max(df2$avg_step), ]$avg_step), "steps."))
```
...
```{r}
colSums(is.na(df))
print(paste("There was", colSums(is.na(df))[1], "rows contained NA."))
```
